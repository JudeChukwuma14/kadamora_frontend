    ...
    >
        ...
        <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
                <span className="font-semibold text-[#0A2D50] text-[15px] truncate max-w-[120px]">{name}</span>
                {subtitle && (
                    <span className="text-xs text-[#64748B] font-medium truncate max-w-[80px]">{subtitle}</span>
                )}
            </div>
            <div className="text-xs text-[#64748B] truncate max-w-[180px]">{lastMessage}</div>
        </div>
        <div className="flex flex-col items-end min-w-[56px] ml-2">
            <span className="text-xs text-[#98A2B3] mb-1">{time}</span>
            {unreadCount > 0 && (
                <span className="inline-flex items-center justify-center h-5 min-w-[20px] px-1.5 rounded-full bg-[#0A66B2] text-white text-xs font-semibold">
                    {unreadCount}
                </span>
            )}
        </div>
